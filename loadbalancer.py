...
class LoadBalancer(object):
    ...
    def on_accept(self):
        ...
        print ('✅ — New client connected %s `%s.' % (c_address, self.cs_socket.getsockname()))
        ...
        try:
            ...
            print ('✅ — Connected to server %s %s.' % (ss_socket.getsockname(),(socket.gethostbyname(server_ip), server_port)))
        except:
            print ("💩 — Couldn't establish connection with server socket, err: %s" % sys.exc_info()[0])
    ...
    def on_receive(self, sock, data):
        ...
        print ('✅ — Sending packets: %-20s ==> %-20s' % (remote_socket.getsockname(), remote_socket.getpeername()))

    def on_close(self, sock):
        print ('💩 — Client %s has disconnected from Load Balancer.' % (sock.getpeername(),))
...
if __name__ == '__main__':
    try:
        LoadBalancer(config.WS_SERVER_HOST, config.LOAD_BALANCER_SERVER_PORT, 'round robin').start()
